...
Download `mnist_train.csv` and `mnist_test.csv` from the [MNIST dataset page](https://git-disl.github.io/GTDLBench/datasets/mnist_datasets/) and place them under `PaperExample/Data/MNIST_CSV/`. From the `PaperExample` directory, the following extension to `mapper.R` saves the graph used here:

```{r mnist-save, eval=FALSE}
source("mapper.R")
...
```{r mnist-static, echo=FALSE, out.width="82%", fig.align="center"}
